...
////////////////////////////////////////////////////FOR PAGES///////////////////////////////////////////////////////////
Future<List<String>> getNextPage() async{
  print (pageList.length);
  Response response = await get(
      createQueryString() + "page=" + (pageList.length + 1).toString());
  Map data = jsonDecode(response.body);
  List<String> temp = [];
  List<dynamic> yes = data['results'];
  if(yes != null)
...
void renderPagesStart() async {
  Future totalPages;
  totalPages = getNextPage();
  totalPages.then((value) => renderPagesEnd(value));
}

void renderPagesEnd(List<String> isNotFinalPage)
{
  if(isNotFinalPage.isNotEmpty)
    {
...
List<Widget> renderPageList()
{
  ...
  if(pageList.isNotEmpty){
    for(int i = 1; i <= pageList.length; i++)
      {
        tempList.add(FlatButton(onPressed: (){goToPage(i);}, child: Text(i.toString())));
      }
  }
  return tempList;
}
...
{
  setState(() {
    animeList = pageList[pageNumber - 1];
  });
}
////////////////////////////////////////////FOR PAGES///////////////////////////////////////////////////////////////////
...
//creates the query to use for GET based on what is in the filter lists. Self explanatory. If all filterlists are empty, then does a default get of currently airing shows
  ...
  Future<List<String>> getSearchInfo (String searchValue) async
  {
    String searchString = '';
    (searchValue.isEmpty) ? searchString = 'https://api.jikan.moe/v3/search/anime?status=airing&order_by=members&' : searchString = 'https://api.jikan.moe/v3/search/anime?q=' + searchValue.replaceAll(' ', '/');
    Response response = await get(searchString);
    Map data = jsonDecode(response.body);
    List<dynamic> yes = data['results'];
    List<String> temp = [];
    yes.forEach((element) => temp.add(element['image_url']));
    ...
    Widget build(BuildContext context) {
      return
        Scaffold(
          ...
          TextField(controller: textController, decoration: InputDecoration(
            ...
            List<List<String>> tempPageList = pageList;
            tempPageList.clear();
            Future testFuture = getSearchInfo(Text);
            testFuture.then((value) => setState((){animeList = value;}));
            testFuture.then((value) => renderPagesStart());
            },
          ),
