...
class _UserPageState extends State<UserPage> {
  ...
  changeName(String newValue) async {
    await FirebaseAuth.instance.currentUser.updateProfile(displayName: newValue);
  ...
  changeComments() async {
    DocumentSnapshot tempShot = await FirebaseFirestore.instance.collection('users').doc(widget.pid).get();
    for(int i = 0; i < tempShot['commentList'].length; i++)
      {
        DocumentSnapshot tempAnime = await FirebaseFirestore.instance.collection('comments').doc(tempShot['commentList'][i]).get();
        for(int j = 0; j < tempAnime['comments'].length; j++)
          {
            if(tempAnime['comments'][j].containsValue(widget.pid)){
              ...
              print(tempMap);
              await FirebaseFirestore.instance.collection('comments').doc(tempShot['commentList'][i]).update({'comments' : FieldValue.arrayRemove([tempMap])});
              tempMap.update('userName', (value) => FirebaseAuth.instance.currentUser.displayName);
              tempMap.update('image', (value) => FirebaseAuth.instance.currentUser.photoURL);
              print(tempMap);
              await FirebaseFirestore.instance.collection('comments').doc(tempShot['commentList'][i]).update({'comments' : FieldValue.arrayUnion([tempMap])});
              break;
            }
          }
      }
